...
habituated_groups <- c("AK", "NH", "BD", "KB", "LT", "CR") # For these groups
{
SEQ_list <- list()
OT_list <- list()
pres_list <- list()
...
for (gp in habituated_groups) {
  ...
  individuals <-  presence_data %>% filter(Group == gp) %>%
  ...
  # presence
  pres_subset <- presence_data %>%  rename(date = Date) %>% mutate(date = ymd(date)) %>%
    filter(Group == gp,
           date < date1,
           date > date2) %>%
